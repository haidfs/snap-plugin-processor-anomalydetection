...
3.在运行snap框架的服务器cd $GOPATH
3.# cd src/github.com/intelsdi-x/snap-plugin-processor-anomalydetection/
4.将snap-plugin-processor-anomalydetection/anomalydetection下的两个go文件替换为该工程anomalydetection/下的两个go文件。
4.cd.. 再make可得二进制文件
5.运行snap框架的服务器首先需要定义的路径结构，例如我是放在了/opt目录下。
...
6.把之前编译得到的二进制文件拷贝到对应路径下：
cp build/linux/x86_64/snap-plugin-processor-anomalydetection /opt/snap_plugin_repos_binary/
7.切换到对应路径
# cd /opt/snap_plugin_repos_binary/
8.执行启动脚本
# ./anomaly.sh
9.约20分钟后，到/tmp/拉取log文件到windows本机。
10.根据Python工程parseSnapAnomaly绘图。
Python工程下的两个Python文件：
10.1 anomalyDataParseAndContrast.py  在运行snapteld一整套插件后，从linux上的/tmp文件夹拉取两个对比的log文件到windows本机，
具体文件名在两个task Manifest中进行定义。根据两个对比文件，可以画出近似https://github.com/intelsdi-x/snap-plugin-processor-anomalydetection的对比图。
10.2 tukeyContrastOnlyFromRawData.py  根据tukey's test的异常检测原理，直接根据原始数据的log文件，而不需要anomaly-processor处理后的log文件，
直接画出对比图。
